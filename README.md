...
Це ETL скрипт який обробляє данні про книги з PostgreSQL бази (Neon). Він витягує нові записи книг, трансформує їх по бізнес-правилам і зберігає результат в таблицю books_processed.
...
У тебе вже є файл `.env` з креденшалами, але якщо нема - створи його і заповни такими данними:
...
- `DB_HOST` - це адреса твоєї бази  
- `DB_PORT` - порт для підключення
- `DB_NAME` - назва бази
- `DB_USER` - юзернейм
- `DB_PASSWORD` - пароль 

### 4. Встановити python-dotenv
Щоб скрипт читав .env файл:
```bash
pip install python-dotenv
```

### 5. Створити таблиці в базі
Треба запустити SQL скрипт `books_schema.sql` 


## Як запустити ETL python скрипт

Просто запускаєш Python файл і передаєш дату з якої треба взяти книги:
